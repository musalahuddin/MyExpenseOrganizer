...
public class TransactionCategoryTable extends Model{

    // URI
    public static final Uri CONTENT_URI = MyExpenseOrganizerProvider.TRANSACTION_CATEGORIES_URI;
    // Table columns
    public static final String TABLE_TRANSACTION_CATEGORY = "transaction_categories";
    ...
    private static final String DATABASE_INSERT = " INSERT INTO " + TABLE_TRANSACTION_CATEGORY + "("+COLUMN_NAME+")"
            + " VALUES(\"\") ;";

    public static void onCreate(SQLiteDatabase database) {
        database.execSQL(DATABASE_CREATE);
        database.execSQL(DATABASE_INSERT);
        CatImporter.importTransactionCategories();
    }

    public static void onUpgrade(SQLiteDatabase database, int oldVersion,
                                 int newVersion) {
        Log.w(TransactionCategoryTable.class.getName(), "Upgrading database from version "
                + oldVersion + " to " + newVersion
                + ", which will destroy all old data");
        database.execSQL("DROP TABLE IF EXISTS " + TABLE_TRANSACTION_CATEGORY);
        onCreate(database);
    }
