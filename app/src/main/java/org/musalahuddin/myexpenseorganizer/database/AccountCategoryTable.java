...
public class AccountCategoryTable extends Model{

    // URI
    public static final Uri CONTENT_URI = MyExpenseOrganizerProvider.ACCOUNT_CATEGORIES_URI;
    // Table columns
    public static final String TABLE_ACCOUNT_CATEGORY = "account_categories";
    ...
    public static void onCreate(SQLiteDatabase database) {
        ...
        CatImporter.importAccountCategories();
    }

    public static void onUpgrade(SQLiteDatabase database, int oldVersion,
                                 int newVersion) {
        Log.w(AccountCategoryTable.class.getName(), "Upgrading database from version "
                + oldVersion + " to " + newVersion
                + ", which will destroy all old data");
        database.execSQL("DROP TABLE IF EXISTS " + TABLE_ACCOUNT_CATEGORY);
        onCreate(database);
    }
